...
import TrackEntryTime from './TrackEntryTime';
...
function TrackEntry(props) {
    const { divider, id, member, description, time, onActionClick } = props;
    ...
    const handleToggleTimer = () => {
        if (timerIsRunning) {
            ...
            const currentTimeEntryRef = db.collection('trackers').doc(timerRef.id); // time entry of a now-stopped timer
            ...
            if (timerRef.id !== id) {
                const timeEntryRef = db.collection('trackers').doc(id);
            ...
            <ListItemText
                primary={
                    <span>
                        ...
                        <TrackEntryTime timerIsRunning={thisTimerIsRunning} timerValue={timerValue} time={time} />
                    </span>
                }
